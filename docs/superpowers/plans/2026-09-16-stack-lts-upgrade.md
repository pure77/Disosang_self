...
작성일: 2026-09-16
상태: **완료** (0~7단계, 2026-09-16 ~ 2026-09-18). 8단계 Boot 4는 별도 계획으로 이관.
...
| Spring Boot | 3.5.5 | 3.5 계열 구 패치. **3.5 OSS 지원은 2026-06-30 종료** (endoflife.date 기준, 3.5.16이 마지막 OSS 릴리스) | 3.5.16 (같은 계열 최신) | OSS 종료. 상용 지원 2032-06-30. → Boot 4.1 (OSS 2027-07-31까지, Java 17~26) 전환 필요 |
| Gradle | 8.14.3 | Java 25 toolchain은 9.1+ 필요 | 9.x 최신 | - |
| Lombok | BOM 관리 | JDK 25 지원 버전 필요 | 명시 고정 | - |

Boot 3.5 OSS 지원 종료일 확인 결과(2026-09-18, endoflife.date): **2026-06-30, 이미 종료.** 3.5.16(2026-06-25)이 마지막 OSS 패치이고 이후 보안 패치는 상용 지원(2032-06-30)에서만 나온다.
따라서 8단계(Boot 4)는 "나중"이 아니라 **이미 기한이 지난 작업**이다. 목표는 Boot 4.0(OSS 2026-12-31 종료)이 아니라 **4.1.x**(2026-06-30 출시, OSS 2027-07-31까지, Java 17~26 지원)로 잡는다.
...
### 7단계. 문서 (PR 1개, 0.5일) — 완료 2026-09-18

**목적**: README와 실제를 일치시키고 업그레이드 기록을 남긴다.

수행
- README 배지: Java 25 LTS, Spring Boot 3.5.16, Gradle 9.7, MySQL 8.4 LTS, Flyway, Docker
- README "실행 방법" 절 신설: compose로 MySQL 8.4 기동 → secret 파일 → bootJar → `java -jar`. Flyway 자동 스키마, `disosang/sql/`은 이력용. 부하 테스트는 jar 실행 조건 명시
- README "기술 스택 LTS 업그레이드" 절 신설: 단계별 변경·확인·p95·PR 표, 플랫폼 발견 요약
- README 성능 표 각주에 측정 환경(Windows MySQL 8.0 서비스 vs Docker MySQL 8.4 = 54ms) 명시
- 이 파일: 상태 완료, Boot 3.5 OSS 종료일 기입, §5 실제 소요, §7 로그, §8 후속 과제 갱신
- 보류: `hibernate.dialect` 명시 제거, `Store` 엔티티 Lombok `@Builder` 경고 3건 → 코드 변경이라 문서 PR과 분리, 후속 과제로

### 8단계. Spring Boot 4.1 (별도 프로젝트, **기한 이미 경과** — 3.5 OSS 지원 2026-06-30 종료)

- 이 계획 범위 밖. Framework 7, Security 7, Hibernate 7, Jackson 3, Tomcat 11이 동시에 바뀌는 메이저 전환.
- 목표 버전은 4.0이 아니라 **4.1.x** (OSS 2027-07-31까지). 4.0은 2026-12-31에 OSS 종료라 곧 같은 문제가 반복된다.
- Java 25, Gradle 9.7, MySQL 8.4, Flyway는 이미 끝나 있으므로 그때는 프레임워크만 본다. 출발점은 이 계획의 완료 커밋.
- 현재 상태의 위험: 3.5.16 이후 보안 패치가 OSS로 나오지 않는다. 배포 전에 8단계를 끝내는 것을 권장.
- 착수 시 별도 계획 파일 작성. 같은 원칙(PR 하나에 변경 하나, k6 3회 ≤ 기준점 +15%). 새 기준점은 Docker MySQL 8.4 기준 54ms.
...
| 단계 | 예상 | 실제 (작업 시간 기준) |
|---|---|---|
| 0. 사전 정리 | 0.5일 | 0.25일 (yml 수정은 5줄, 대부분 세션 환경 문제 진단) |
| 1. Flyway | 1일 | 0.5일 |
| 2. 기준점 | 0.5일 | 1일 (C1 전용 JIT 재측정, 434ms 원인 조사, curl 인코딩 함정) |
| 3. Gradle 9 | 0.5~1일 | 0.5일 (deprecation 0건, 데몬 공유 문제 1회) |
| 4. Boot 3.5.16 | 0.5일 | 0.25일 |
| 5. JDK 25 | 1일 | 0.5일 (재부팅 직후 재측정 1회) |
| 6. MySQL 8.4 | 1일 | 0.5일 (+ 플랫폼 원인 분리 벤치 0.25일) |
| 7. 문서 | 0.5일 | 0.25일 |
| 예비 | 1일 | — |
| **합계** | **6.5~7일** | **약 4일** (달력 기준 2026-09-16 ~ 09-18, 3일) |

예비 1일은 3단계(Gradle 메이저)와 5단계(JDK) 중 하나는 예상을 넘긴다고 보고 넣었다.
실제로는 코드 단계(3~5)가 예상보다 짧았고, 시간이 든 곳은 **측정 조건을 바로잡는 일**(2단계)이었다. 다음 업그레이드 견적에서는 "기준점 확보"를 1일로 잡는다.
...
| 문서 | 2026-09-18 | — | README 배지·실행 방법·업그레이드 요약·측정 환경 각주, 계획 파일 완료 처리, Boot 3.5 OSS 종료일(2026-06-30) 기입 | 풀 20 실험(776ms, 변화 없음)을 참고 자료로 추가. dialect 제거 등 코드 정리는 별도 PR로 분리 | 0.5일 → 0.25일 |
...
3. **README 수치 정정** — 7단계에서 처리: 3차 수치를 712ms(Windows 서비스)로 정정하고 측정 조건과 Docker 8.4 기준 54ms를 각주로 명시. 남은 것: 0d531b5를 jar로 띄워 434ms 재현 여부 확인(선택).
...
6. **DB 플랫폼이 수치를 지배함 (2026-09-18 발견)**: 기준점 739ms부터 5단계 807ms까지 전부 Windows MySQL 8.0 서비스 위에서 잰 값. 같은 8.0.36을 Docker(Linux)로 띄우면 동시 10접속 기준 2.4배 빠르고, 그 위에서 k6 p95는 54ms. 이후 모든 성능 수치는 Docker MySQL 8.4 기준으로 통일한다. 5단계의 807ms(JDK 25 회귀 의심)도 DB 병목 상태에서의 노이즈로 판단, 추가 조사 불필요.
7. **커넥션 풀 크기**: Windows 8.0에서 풀 10 → 20으로 늘려도 p95 807 → 776ms, 처리량 55 req/s 그대로(`2026-09-18_ref-mysql80win-pool20_*`). 병목은 풀이 아니라 DB 처리 용량이었다. 풀 크기는 배포 환경에서 Actuator로 `hikaricp.connections.pending`을 노출한 뒤 VU 300 이상으로 10/20/30을 비교해 정한다. 지금은 기본값 10 유지.
8. **코드 정리 (소규모 PR)**: `application.yml`의 `hibernate.dialect` 명시 제거(Hibernate 6.6 deprecation 경고), `Store` 엔티티 `@Builder` 초기값 경고 3건(`@Builder.Default`), `users` 테이블의 중복 unique 인덱스 `email_2`, `store`의 미사용 컬럼 `category/x/y` 정리(V2 마이그레이션).
9. **store 파생 컬럼 생성 컬럼화 (V2)**: `location`(lon/lat로부터), `place_name_search`, `store_type_search`를 STORED 생성 컬럼으로. 현재는 JPA insert가 `location` NOT NULL로 실패하고, 두 검색 컬럼은 새 데이터에서 NULL로 남는다. CSV 로더(`StoreBatchService`) 동작 회복이 목적.
10. **8.0 Windows 서비스 정리**: 7단계 완료 후 `MySQL80` 서비스 중지, 1주간 문제 없으면 제거. 로컬 secret은 이미 3307. 다음 `bootJar`부터 jar에도 3307이 들어감.
11. **Boot 4.1 전환**: 8단계 참고. 3.5 OSS 지원이 2026-06-30에 끝났으므로 배포 전에 처리.
